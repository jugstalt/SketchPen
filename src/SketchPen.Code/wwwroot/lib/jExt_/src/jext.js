  ...
  class jExtObject {
    ...
    prepend(content) {
      ...
      this._((element) => {
        ...
        contentElements._((contentElement) => {
          element.insertBefore(contentElement.cloneNode(true), firstChild);
        });
      });
    ...
    on(eventNames, handler) {
      ...
      eventsArray.forEach((eventName) => {
        this._((element) => {
          ...
          element._jExtHandlers = element._jExtHandlers || new WeakMap();
    ...
    bind(eventNames, callback) {
      const eventsArray = eventNames.split(" ").map((event) => event.trim());
    ...
    off(eventNames, handler) {
      ...
      eventsArray.forEach((eventName) => {
        this._((element) => {
          if (element._jExtHandlers && element._jExtHandlers.has(handler)) {
            const actualHandler = element._jExtHandlers.get(handler);
      ...
      if (coordinates === undefined) {
        ...
        this.each((i, el) => {
          ...
          $el.css({
            ...
            left: position.left + (coordinates.left - currOffset.left), //+ 'px'
          });
        });
    ...
    position() {
      ...
      const offsetParent = this[0].offsetParent || document.documentElement;
    ...
    serialize() {
      ...
      this._((element) => {
        if (
          (element.tagName.toLowerCase() === "form" && element.elements) ||
      ...
      for (const key in obj) {
        if (Object.prototype.hasOwnProperty.call(obj, key)) {
          if (
            ...
            target[key] = this.extend(true, target[key] || {}, obj[key]);
          } else {
            target[key] = obj[key];
          }
        }
